    ...
    <div className="fixed z-50 top-0 left-0 right-0 transition-all duration-300">
              ...
              <div className="text-textWhite relative pb-[2px] after:absolute after:left-0 after:bottom-[-2px] after:w-0 after:h-[2px] group">
                ...
                <div className="hidden group-hover:flex">
                  <div className="w-[172px] h-[185px] p-5 text-[14px] text-textBlack absolute bg-textWhite font-semibold rounded-xl flex flex-col gap-[15px] mt-1">
                    ...
                    <Link href="/projects/non-residential">{t.nonResidential}</Link>
                  </div>
                </div>
              </div>

              <Link href="/" className="text-textWhite hover:after:w-full relative pb-[2px] after:absolute after:left-0 after:bottom-[-2px] after:w-0 after:h-[2px] after:bg-textWhite after:transition-all after:duration-200">
                {t.articles}
              </Link>

              <Link href="/about" className="text-textWhite hover:after:w-full relative pb-[2px] after:absolute after:left-0 after:bottom-[-2px] after:w-0 after:h-[2px] after:bg-textWhite after:transition-all after:duration-200">
                {t.about}
              </Link>

              <Link href="/social" className="text-textWhite hover:after:w-full relative pb-[2px] after:absolute after:left-0 after:bottom-[-2px] after:w-0 after:h-[2px] after:bg-textWhite after:transition-all after:duration-200">
                {t.social}
              </Link>
            </div>
            ...
            <div className="hidden lg:flex">
              <div className="flex gap-4 justify-between items-center text-[22px]">
                <button
                  ...
                  className={language === "uz" ? "text-textWhite font-semibold" : "text-lightGrey"}
                >
                  {t.uz}
                </button>
                <button
                  onClick={() => toggleLanguage("en")}
                  className={language === "en" ? "text-textWhite font-semibold" : "text-lightGrey"}
                >
                  {t.en}
                </button>
                <button
                  onClick={() => toggleLanguage("ru")}
                  className={language === "ru" ? "text-textWhite font-semibold" : "text-lightGrey"}
                >
                  {t.ru}
                </button>
              </div>
      ...
      {isMenuOpen && (
        <div className="lg:hidden fixed top-0 left-0 w-full h-full bg-black bg-opacity-90 z-40 flex flex-col pt-8 items-center text-textWhite text-xl font-semibold gap-y-4">
          ...
          <Link href="/" className="pt-12" onClick={() => setIsMenuOpen(false)}>
            {t.home}
          </Link>
          <Link href="/projects/all" onClick={() => setIsMenuOpen(false)}>
            {t.allProjects}
          </Link>
          <Link href="/projects/multi-storey" onClick={() => setIsMenuOpen(false)}>
            {t.multiStorey}
          </Link>
          <Link href="/projects/low-rise" onClick={() => setIsMenuOpen(false)}>
            {t.lowRise}
          </Link>
          <Link href="/projects/non-residential" onClick={() => setIsMenuOpen(false)}>
            {t.nonResidential}
          </Link>
          ...
          <div className="flex gap-4 pt-6">
            <button onClick={() => toggleLanguage("uz")} className="text-white">{t.uz}</button>
            <button onClick={() => toggleLanguage("en")} className="text-lightGrey">{t.en}</button>
            <button onClick={() => toggleLanguage("ru")} className="text-lightGrey">{t.ru}</button>
          </div>
